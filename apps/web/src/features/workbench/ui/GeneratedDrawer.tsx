...
const s = stylex.create({
  /* The workbench side panel hosts this view inside its tab panel, so the Runs
   * drawer fills the box the panel gives it rather than docking to a window
   * edge or floating over the canvas of its own accord. Same surface token as
   * the panel, so the tab and its body read as one pane. */
  view: {
    minHeight: 0,
    flex: 1,
    display: "flex",
...
export function GeneratedDrawer({
  ...
  return (
    <div role="region" aria-label="Generated" {...stylex.props(s.view)}>
