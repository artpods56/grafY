...
async function center(locator: Locator) {
  ...
  if (!bounds)
    throw new Error(`Canvas element has no visible bounds: ${locator}`);
  return { x: bounds.x + bounds.width / 2, y: bounds.y + bounds.height / 2 };
}

test("one finger pans without starting a selection", async ({
  page,
  hasTouch,
}) => {
  ...
  for (const distance of [20, 40, 60, 80]) {
    await touch(page, client, "touchMove", [
      { x: point.x + distance, y: point.y + distance, id: 1 },
    ]);
    await expect(page.locator(".react-flow__selection")).toHaveCount(0);
  }
  ...
  await expect
    ...
    .toMatchObject({ zoom: before.zoom });
...
test("a pan can become a pinch and return to one finger", async ({
  page,
  hasTouch,
}) => {
  test.skip(!hasTouch, "Requires a touch-enabled browser");
  ...
  await expect
    .poll(async () => (await viewportTransform(page)).zoom)
    .toBeGreaterThan(before.zoom + 0.1);
  expect(await page.evaluate(() => window.visualViewport?.scale)).toBe(1);

  // A targeted CDP touchEnd releases finger 2 while finger 1 stays down.
  await touch(page, client, "touchEnd", [
    { x: point.x + 60, y: point.y + 10, id: 2 },
  ]);
  const afterPinch = await viewportTransform(page);
  await touch(page, client, "touchMove", [
    { x: point.x - 30, y: point.y + 40, id: 1 },
  ]);
  await touch(page, client, "touchEnd", []);
...
test("cancelling a touch gesture leaves the next drag usable", async ({
  page,
  hasTouch,
}) => {
  test.skip(!hasTouch, "Requires a touch-enabled browser");
...
test("mouse drag selection and right-button panning remain available", async ({
  page,
}) => {
  const point = await center(page.locator(".react-flow__pane"));
  const before = await viewportTransform(page);
...
test("touch can select and move a node without moving the viewport", async ({
  page,
  hasTouch,
}) => {
  test.skip(!hasTouch, "Requires a touch-enabled browser");
...
test("ports still connect through a real drag", async ({ page, hasTouch }) => {
  ...
  if (hasTouch) {
    ...
    for (const distance of [40, 80, 120, 160, 200]) {
      await touch(page, client, "touchMove", [
        { x: header.x, y: header.y + distance, id: 1 },
      ]);
    }
    await touch(page, client, "touchEnd", []);
  } else {
