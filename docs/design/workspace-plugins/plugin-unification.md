...
- **Related:** [workspace-plugins index](README.md),
  [agent-authored nodes](../agent-authored-nodes.md) (current prototype),
  [plugin development](../plugin-development.md) (current in-process host
  adapter), [modules conceptual model](../modules-conceptual-model.md),
...
[`examples/plugin-notes`](../../../examples/plugin-notes) is a small uv project
that follows this note: slug `notes`, two `function_node`s, `table.data@1` and
`scalar.text@1` from core (no `grafy-plugin-gis`), a plugin-owned
`notes.table_summary@1` with writer and resolver, profile `python-uv`, and
**no** `grafy.plugins` entry point. FastAPI does not load it.
