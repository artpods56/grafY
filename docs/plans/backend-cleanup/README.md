...
- [x] Route synchronous `POST /runs` through manager-owned durable activity while preserving response, error, cancellation, and admission behavior.
- [x] Retain maintenance protection when sandbox cleanup raises or is interrupted before container removal is confirmed; verify both transient and saved executions.
- [x] Re-audit every execution entry point and prove active execution/revocation races cannot violate the fence for the supported single-owner deployment.
...
### Synchronous execution activity

- `RunExecutionManager.run_inline` registers durable activity before preflight and waits for the existing executor, including sandbox cleanup, in the caller task. It propagates execution errors and cancellation directly and creates no saved execution history or pollable background record. The route retains its existing admission lease through response presentation. [R01: Direct Ownership]
- Removed the unused `RunGraphDependency` and its test override. Both top-level calls to `RunGraph.run` now live in the manager. The remaining `execute_module` call is in the core module operator and runs within its parent's execution lifetime. Catalog injection constructs module contracts; it does not execute a graph. [R17: Delete Dead Abstractions]
- Registration failure, including uncertain commit acknowledgement and cancellation, prevents execution and attempts marker cleanup. Removal retries once. Persistent removal failure retains the marker and reports the execution identity; an existing execution error keeps precedence and the cleanup error is logged. [R42: Errors Carry Context]
- Real SQL race coverage now exercises the synchronous route handler as well as background management. It verifies revocation-before-admission, active preflight, invocation, sandbox cleanup, and synchronous cancellation during cleanup. The HTTP route still returns 422 for a revoked release, returns capacity after termination, and preserves capacity through successful and failed response presentation. [R43: Tests Are Behavioral Contracts]
- Verification: 44 database race/lifecycle tests pass using SQLite and disposable PostgreSQL. Targeted manager/dependency/test types report zero errors; the direct route-handler import has one narrow documented suppression because FastAPI's capability factory constructs the access annotation at runtime. Ruff passes. Application registration and OpenAPI match the checked-in schema exactly. [R11: Framework Constraints Must Be Explicit]
- The broad API/execution/architecture run passed 557 tests; one Docker image export failed with host `ENOSPC`. Extracted-wheel verification initially hit SQLite disk I/O errors during the same disk exhaustion. Removed only the completed audit run's disposable `pytest-1584` directory, identified by its log. The wheel rerun then passed 47 tests with 9 PostgreSQL cases skipped, verified wheel import paths, and reproduced exact OpenAPI. The failed Docker sandbox test passed on its targeted retry, bringing the broad regression result to 558 passing tests across the original run and retry. PostgreSQL cases passed separately in the 44-test run. Existing native macOS crash exclusions remain `test_plugin_egress_docker.py` and `test_workspace_plugin_protocol.py`.
- Evidence: `/tmp/grafy-inline-postgres.log`, `/tmp/grafy-inline-api-regression.log`, `/tmp/grafy-inline-types.log`, `/tmp/grafy-inline-wheel-retry.log`, `/tmp/grafy-inline-docker-retry.log`, and `/tmp/grafy-inline-build.log`.
- Finding 3 remains open. Inspection of `DockerPluginRuntime.close_scope` found that resource removal may raise while a container remains, whereas manager terminal handling currently removes activity after executor failure. The next change must preserve maintenance protection on unconfirmed cleanup for both transient and saved executions and test the failure through the real executor. Graph transport migration and the final completion gates also remain open.


### Confirmed cleanup and retained maintenance activity

- Reproduced six failures through the real executor and SQL release service: failed or interrupted sandbox cleanup allowed revocation after inline, transient-background, and saved runs. The regression now keeps revocation and cutover blocked until cleanup is confirmed. [R43: Tests Are Behavioral Contracts]
- Added `PluginSandboxCleanupError` at the sandbox lifecycle interface. It carries the scope identity and chains the original failure. `RunGraph` distinguishes caller cancellation after successful cleanup from failed or interrupted cleanup. The manager reports the latter as a failed local result and retains the marker and active saved history. [R42: Errors Carry Context]
- Saved execution now registers an activity marker after claiming its queue row and before preparation. This closes the restart gap: terminalizing saved history cannot bypass the guarded activity recovery step. The existing marker table contains identity and ownership only; saved queue/history contracts and schema remain unchanged. Maintenance reports deduplicate executions represented in both tables. [R01: Direct Ownership]
- Added 22 real-executor/database cases across SQLite and PostgreSQL. They cover successful cleanup, failures, worker cancellation, caller cancellation during cleanup, repeated cancellation, preserved error context, deduplicated revocation/cutover reporting, saved-history interruption, and guarded recovery. Normal completion still releases activity and saves successful history.
- Re-audited top-level execution calls. The only production `RunGraph.run` callers are manager inline/background paths. HTTP execution routes enter the manager. The core module operator's `execute_module` call stays inside its parent's scope. Composition supplies execution history; catalog injection constructs contracts and does not execute graphs. Finding 3 is complete for the supported single-owner deployment.
- Verification: 558 API/execution/architecture tests pass, including the Docker sandbox regression. Persistence passes 199 tests with 41 optional PostgreSQL cases skipped in that run; the dedicated disposable PostgreSQL/SQLite run passes all 66 cleanup, race, lock-ordering, and lifecycle cases, covering 22 of those skipped cases. The 19 older optional PostgreSQL cases remain outside this batch. All changed production modules and the new tests have zero Pyright errors; Ruff passes.
- Built API and persistence wheels offline and verified imports resolve to their extracted contents. Packaged-code checks pass 58 tests with 20 PostgreSQL cases skipped, covered separately by the dedicated database run. Application registration and OpenAPI match the checked-in schema exactly. Evidence: `/tmp/grafy-cleanup-fence-wheel.log`.
